...
struct Piece {
  ...
  bool operator<(const Piece &other) const {
    return std::tie(colour, type) < std::tie(other.colour, other.type);
  }
};
...
bool PawnMove(int x1, int x2, int y1, int y2, ChessBoard &chessBoard) {
  ...
  if (abs(xSteps) == 1 && abs(ySteps) == 1 &&
    ...
    if (targetSquareY >= 0 && targetSquareY < 8) {
      ...
      if (targetPiece.enp == EnPassant::TRUE &&
          targetPiece.colour != source.colour) {
...
bool LegalMoves(ChessBoard &chessBoard) {
  for (int i = 0; i < 8; i++) {
    for (int j = 0; j < 8; j++) {
      if (WhoMove() &&
          chessBoard.getPieceAt(i, j).colour != PieceColour::WHITE) {
        continue;
      } else if (!WhoMove() &&
                 chessBoard.getPieceAt(i, j).colour != PieceColour::BLACK) {
        continue;
      }
      for (int x = 0; x < 8; x++) {
        for (int y = 0; y < 8; y++) {
          if (MoveCheck(i, j, x, y, chessBoard)) {
            return true; // there is legal moves
          }
        }
      }
    }
...
bool stalemate(ChessBoard &ChessBoard) {

  if (!isInCheck(ChessBoard) && LegalMoves(ChessBoard)) {
...
bool doMove(ChessBoard &chessBoard, int x1, int x2, int y1, int y2) {

  std::cout<<"Running doMove\n";
  Piece source = chessBoard.getPieceAt(x1, y1);
...
void SaveBoardState(ChessBoard &chessBoard) {

  for (int i = 0; i < 8; i++) {
    for (int j = 0; j < 8; j++) {
      chessBoard.PreviousBoardState[i][j] = chessBoard.getPieceAt(i, j);
    }
  }
}
...
void ResetEnPassant(ChessBoard &chessBoard) {
  for (int i = 0; i < 8; i++) {
    Piece &piece =
        chessBoard.getPieceAt(i, WhoMove() ? 3 : 4); // Check the relevant rank
